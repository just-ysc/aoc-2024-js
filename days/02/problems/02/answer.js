...
const isSafe = report => {
  ...
  for (let i = 1; i < report.length; i++) {
    ...
    if (prevLevel === curLevel) {
      return (
        isToleratedSafe(report.filter((_, idx) => idx !== (i - 2)))
        || isToleratedSafe(report.filter((_, idx) => idx !== (i - 1)))
        || isToleratedSafe(report.filter((_, idx) => idx !== i))
      );
    }
    ...
    if (Math.abs(prevLevel - curLevel) > 3) {
      return (
        isToleratedSafe(report.filter((_, idx) => idx !== (i - 2)))
        || isToleratedSafe(report.filter((_, idx) => idx !== (i - 1)))
        || isToleratedSafe(report.filter((_, idx) => idx !== i))
